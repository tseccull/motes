...
def data_harvest(intreg, filename_2D, chunk, param_dict):
    ...
    with fits.open(filename_2D) as imgfile:
        ...
        imgdata, imgerrs, imgqual, ogimgqual, head_dict, wavaxis = instrument_dict[inst](
            imgfile, imghead)

    # SLICE ALL DATAFRAMES BASED ON THE INPUT FROM reg.txt. (i.e. defining
    # spatial pixel rows within image)
    ...
    # Create spatial axis for the 2D spectrum and a high resolution version
    # (standard res * 5) for the purposes of plotting
    ...
    # TODO - Notify the user which limits are wrong.
    ...
    # These are simple masks one can apply later
    ...
    sys.stdout.write(' >>> 2D spectrum sliced on dispersion axis based on user defined limits:\n'
                     ...
                     str(chunk[intreg][2]) + '-' + str(chunk[intreg][3]) +
                     ' ' + head_dict['wavunit'] + '.\n'
...
def harvest_gmos(imgfilehdu, imgheader):
    ...
    sys.stdout.write('DONE.\n')
    sys.stdout.write(' >>> Spatial pixel resolution determined: ' +
...
def harvest_xshoo(imgfilehdu, imgheader):
    ...
    # Convert qual2D to binary, ensuring that extrapolated pixels are not bad.
    # Refer to the function for details.
    imgqual = common.qual_to_molecfit_compat(imgqual)
